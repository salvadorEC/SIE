<?php
  ...
  $acentos = $mysqli->query("SET NAMES 'utf8'");
    ...
    $Ver_Acreditaciones= "SELECT Id_Acreditacion,No_Lote,Ano_Acreditacion,No_Oficio,Periodo,Fecha_Acreditacion,Matricula_Acreditacion,Nombre_Alumno,Idioma,Nivel_Acreditacion,Carrera_Alumno FROM $ACREDITACIONES JOIN $ALUMNOS ON  Matricula_Alumno = Matricula_Acreditacion  WHERE Fecha_Acreditacion = '$Fecha_Por_Lote' ORDER BY Nombre_Alumno ASC  ";
    ...
    function Header()
    {
        ...
        $this->Cell(0,0,'FACULTAD DE CIENCIAS ADMINISTRATIVAS',0,0,'C');
        $this->Ln(20);
...
while ($row = mysqli_fetch_array($Result_Ver_Acreditaciones))
    {
      ...
      $pdf->Cell(0,70,utf8_decode('P r e s e n t e.-'),0,0,'');
      ...
      $pdf->SetFont('Arial','',11);
      $pdf->Text(34,185,utf8_decode('Agradezco de antemano, la atención que se sirva brindar a la presente y sin otro particular'));
      $pdf->Text(24,195,utf8_decode('por el momento, reitero a usted la seguridad de nuestra atenta consideración.'));
      // Logo UABC
      $pdf->Image('../images/logo_carta_acreditacion.png',158,220,30);
      // ############ FECHA LARGA DEL FOOTER #######
      $pdf->SetFont('Arial','B',12);
      $pdf->Text(55,210,utf8_decode('Mexicali,  Baja California, '.$exploded [2].' de '.$exploded [1].' de '.$exploded [0]));
      $pdf->Text(55,218,utf8_decode(('"POR LA REALIZACIÓN PLENA DEL HOMBRE"')));
      $pdf->Text(92,225,utf8_decode(('DIRECTOR')));
      $pdf->Text(72,245,utf8_decode(('DR. RAÚL GONZÁLEZ NÚÑEZ')));
